...
void offset_calu(int value){                            //根据imac索引信息表，对offset进行计算
   // u32 cfg_offset[2];
    ...
    if (table.table[value].sync_seq == 2 ){
	...
	}else if(table.table[value].sync_seq>2 ){
...
void offset_cfg_send(u16 value){
    ...
    printf("guide_offset = %d, guide_offset_flag = %d\n",table.table[value].guide_offset,table.table[value].guide_offset_flag);
    ...
    build_send_chip_cfg_pkt(value,CHIP_OFFSET_PERIOD_ADDR,1,&(frequency));   //发送周期为0的配置报文、只配置一

	u32 cfg_data = 3;
	if(table.table[value].Latest_Offset<100)
	{
		num++;
		if(value == 4)
		{
			if(num>100)
			{
				build_send_chip_cfg_pkt(value,CHIP_CFG_FINISH_ADDR,1,&cfg_data);
			}		
		}
		else
		{
    ...
    if(table.table[value].sync_seq > 1){

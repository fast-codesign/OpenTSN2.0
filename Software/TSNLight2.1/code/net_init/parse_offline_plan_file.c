...
{  
	...
	{  
		...
		{  
			//默认为1，全开
			printf("gate_table\n");
			memset(&chip_cfg_table,0xff,sizeof(chip_cfg_table));
...
*/
	...
	{
		cfg_data = 1000;
		build_send_chip_cfg_pkt(imac,CHIP_REPORT_PERIOD_ADDR,1,(u32 *)&cfg_data);//finish
	}


	cfg_data = 2;
	build_send_chip_cfg_pkt(imac,CHIP_CFG_FINISH_ADDR,1,(u32 *)&cfg_data);//finish
    return 0;  
}

...
u64 discnt_sum_host = 0;
u64 discnt_sum_p[8] = {0};
...
int report_default_cfg(u16 dimac)
{
	...
	build_send_chip_cfg_pkt(dimac,CHIP_REPORT_PERIOD_ADDR,1,(u32 *)&default_report_period);	
	printf("2222222222222222222222\n");
	build_send_chip_cfg_pkt(dimac,CHIP_REPORT_TYPE_ADDR,1,(u32 *)&default_report_type);
	build_send_chip_cfg_pkt(dimac,CHIP_REPORT_EN_ADDR,1,(u32 *)&default_report_en);
...
u16 sum_ts_inj_underflow_error_cnt;
u16 sum_ts_inj_overflow_error_cnt;
u16 sum_ts_sub_underflow_error_cnt;
u16 sum_ts_sub_overflow_error_cnt;
...
void printf_chip_state_info(chip_static *tmp_chip_static)
{
	...
	sum_ts_inj_underflow_error_cnt += tmp_chip_static->chip_module_status.ts_inj_underflow_error_cnt;
	sum_ts_inj_overflow_error_cnt  += tmp_chip_static->chip_module_status.ts_inj_overflow_error_cnt;
	sum_ts_sub_underflow_error_cnt += tmp_chip_static->chip_module_status.ts_sub_underflow_error_cnt;
	sum_ts_sub_overflow_error_cnt  += tmp_chip_static->chip_module_status.ts_sub_overflow_error_cnt;

	printf("sum_ts_inj_underflow_error_cnt 	 %d\n",sum_ts_inj_underflow_error_cnt);
	printf("sum_ts_inj_overflow_error_cnt 	 %d\n",sum_ts_inj_overflow_error_cnt);
	printf("sum_ts_sub_underflow_error_cnt 	 %d\n",sum_ts_sub_underflow_error_cnt);
	printf("sum_ts_sub_overflow_error_cnt 	 %d\n",sum_ts_sub_overflow_error_cnt);
	...
	printf("****************************sum info********************************\n");
	printf("discnt_sum_host 	 %lld\n",discnt_sum_host);
	for(port_idx=0;port_idx<8;port_idx++)
	{
		//printf("port[%d] discnt %d\n",port_idx,discnt_sum_host[port_idx]);
...
int parse_chip_state_report(chip_state_info *state_info,u8 *pkt,u16 len)
{
	int port_idx = 0;
	...
	discnt_sum_host = state_info->chip_report.chip_pkt_static.host_discard_cnt;
	...
	for(port_idx=0;port_idx<8;port_idx++)
	{
		discnt_sum_p[port_idx] = state_info->chip_report.chip_pkt_static.rx_discard_cnt[port_idx].port_discard_cnt;
...
void state_monitor_callback(u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *packet)
{
	...
	//else if(report_type == 2048)
	{
		printf("CHIP_STATE_REPORT\n");
		//获取各个节点的状态
		parse_chip_state_report(&chip[imac].state,(u8*)packet,pkthdr->len);
		//配置上报类型为寄存器上报
